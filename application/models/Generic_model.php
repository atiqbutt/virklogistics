...
class Generic_model extends CI_Model {
           function GetPriceHistory($fd) {
	//		   print("hi model"); die();
        $this->db->select('p.id as id,p.heading as heading,ph.price as price,ph.date as enddate,ph.startdate as startdate')
        ->from('product as p')
		
               ->join('product_histroy as ph','ph.p_id=p.id')
            //    ->join('sales_detail as sd','s.id=sd.sales_id')
        ->where(array('ph.startdate'=>$fd));
	//	$this->db->like($array);
     //  ->order_by("cid","DESC");
        $w=$this->db->get();
        if ($w->num_rows() > 0){
...
//echo "$id";		
	//	die();
        $this->db->select('e.id as id,e.heading as heading')
        ->from('product as e')
//        $a=p.id.poorchild;
//                ->join('unit as u','e.unit_id=u.id')
                ->join('product_histroy as ph','ph.p_id=e.id')
       
        ->where(array('e.is_deleted'=>0,'e.status'=>0,'ph.p_id'=>$id))
        ->order_by("e.id","DESC");
         $w=$this->db->get();
        if ($w->num_rows() > 0){
        return $w->result_array();}
